...
- Use an in-memory mail backend captured by the base test setup (`MemoryMailBackend` collecting messages on a module-level list, swapped in via fixture). Inspect the captured list — do not hit a real SMTP server.
...
|----------------------|----------------------|
| Empty method body (no mail sent) | `assert len(outbox) > 0` fails |
| Wrong template used | Body content mismatch |
| Sync sender called from async context without `await` | `RuntimeWarning: coroutine was never awaited` then assertion failure |
...
- `received_messages()` — returns the captured outbox list
...
- `autouse` fixture cleanup — clears the outbox between tests
