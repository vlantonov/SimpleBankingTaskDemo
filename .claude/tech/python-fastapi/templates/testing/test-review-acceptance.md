...
3. **Prefer dataclass / Pydantic equality** — replace 2+ sequential per-field `assert` calls with `assert actual == expected` (frozen dataclasses and Pydantic models do deep structural comparison by default)
...
5. **Never use `TestClient` in acceptance tests** — acceptance is black-box against the running ASGI server. Use `httpx.Client(base_url=BACKEND_URL)` via the application client class.
...
### GOOD: Dataclass / Pydantic Equality (recursive comparison)
```python
# Replace 2+ per-field assertions with structural equality
assert actual == expected  # frozen dataclasses / Pydantic models compare all fields recursively
```
...
| `assert timestamp is not None` | Bound with `abs(...) < timedelta(minutes=1)` |
...
| 2+ sequential `assert x.field == val` | `assert actual == expected` (dataclass / Pydantic eq) |
